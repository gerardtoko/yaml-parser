...
class App_Parser_Yaml_Loader {
    ...
    protected static $_CODE = array('start' => '#^(__code\()#', 'end' => '#(\))$#');
        ...
        if (!empty($t)) {
            ...
            if (!empty($t['key'])) {
                ...
                if ($t['value'] !== self::ARRAY_VALUE) {
                    ...
                    $parse[] = self::decode($lines, ($indent + self::INDENT));
                }
            }
        }
    }
    ...
    public static function parseValue($value, &$lines) {
        ...
        // Parsing CONST
        if (preg_match(self::CONSTANT, $value)) {
            return self::parseValueConst($value);
        }

        // Parsing EXTENDS
        if (preg_match(self::EXT, $value)) {
            return self::parseValueExtends($value);
        }

        // Parsing CODE
        if (preg_match(self::$_CODE["start"], $value)) {
            return self::parseValueCode($value, $lines);
        }
     ...
     * @return type
     */
    public static function parseValueCode($value, &$lines) {
        ...
        if (preg_match(self::$_CODE['start'], $value)) {
            ...
            //On test s'il exite de fermeture
            if (preg_match(self::$_CODE['end'], $value)) {
                $value = (string) substr($value, 0, -1);
                eval("\$value = $value;");
                return $value;
            } else {
                ...
                //On cherche la fermeture
                foreach ($lines as $key => $line) {
                    if (preg_match(self::$_CODE['end'], $line)) {
                        $value = (string) substr($value, 0, -1);
                        eval("\$value = $value;");
                        return $value;
                    } else {
     ...
     * @return type
     */
    public static function parseValueConst($value) {

        $value = constant(trim(substr(strstr($value, '('), 1, -1)));
        return $value;
    }

    /**
     *
     * @param type $extends_value
     * @param type $lines
     * @return type
     */
    public static function parseValueExtends($value) {

        $extends_value = trim(substr(strstr($value, '('), 1, -1));

        $value = self::parseValueExtendsFilter(self::$_parsed, $extends_value);
        return $value;
    }

    public static function parseValueExtendsFilter($array, $extends_value) {

        foreach ($array as $key => $value) {

            if (!is_int($key) && !is_float($key)) {
                if ($key == $extends_value) {
                    return $value;
                }
            }

            if (is_array($value)) {
                $value = self::parseValueExtendsFilter($value, $extends_value);
                if (!is_null($value)) {
                    return $value;
                }
            }
